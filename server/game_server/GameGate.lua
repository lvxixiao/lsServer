...
function server.username(uid, subid, servername)
    return string.format("%s@%s@%s", 
        b64encode(uid),
        b64encode(servername),
        b64encode(tostring(subid))
    )
end
...
function server.start(conf)
    ...
    -- atomic , no yield
    local do_auth = function(fd, message, addr)
        ...
        if crypt.hmac64(crypt.hashkey(index), u.secret) ~= hmac then
    ...
    local auth = function(fd, addr, msg, sz)
    ...
    local function do_request(fd, message)
        ...
        LOG_INFO("do_requset", session, message)
